...
function camelize(text) {
    ...
    if (p >= 0)
        text = text.substring(0, p) + ':' + text[p + 1].toLowerCase() + text.substring(p + 2);
...
function Element(text, offset) {
    ...
    this.toObject = function (options) {
        ...
        var name = normalizeName(this.name(), options);
        
        if (options.array.indexOf(this.tag()) >= 0 || options.array.indexOf(this.name()) >= 0) {
            result[name] = [];
            
            elements.forEach(function (element) {
                result[name].push(element.toObject(options)[normalizeName(element.name(), options)]);
            });
        }
            ...
            elements.forEach(function (element) {
                ...
                var elemname = normalizeName(element.name(), options);
